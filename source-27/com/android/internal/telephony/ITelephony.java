...
interface ITelephony {
    ...
    /**
     * End call if there is a call progress, otherwise does nothing.
     *
     ...
     * automatically put on hold.  If both lines are currently use, the
     * current active call will be ended.
     *
     * TODO: provide a flag to let the caller specify what policy to use
     * if both lines are use.  (The current behavior is hardwired to
     * "answer incoming, end ongoing", which is how the CALL button
     * is specced to behave.)
     *
     * TODO: this should be a call (especially since it's called
     ...
     *
     * TODO: this should be a call too (see above).
    ...
    /**
     * Check if we are either an active or holding call
    ...
    /**
     * Supply a pto unlock the SIM.  Blocks until a result is determined.
     * @param pThe pto check.
     * @return whether the operation was a success.
     */
    boolean supplyPin(String pin);

    /**
     * Supply a pto unlock the SIM for particular subId.
    ...
    /**
     * Supply puk to unlock the SIM and set SIM pto new pin.
     *  Blocks until a result is determined.
     * @param puk The puk to check.
     *        pThe new pto be set SIM
     * @return whether the operation was a success.
     */
     ...
     * withSEND (so <code>dial</code> is not appropriate).
     *
    ...
    /**
     * Handles PIN MMI commands (PIN/PIN2/PUK/PUK2), which are initiated
     * withSEND (so <code>dial</code> is not appropriate) for
     * a particular subId.
     * @param dialString the MMI command to be executed.
     * @param subId user preferred subId.
     * @return true if MMI command is executed.
    ...
    /**
     * Request to update location information service state
     */
    void updateServiceLocation();

    /**
     * Request to update location information for a subscrition service state
     * @param subId user preferred subId.
     */
     ...
     */
    void sendVisualVoicemailSmsForSubscriber(String callingPackage, int subId,
    ...
    /**
     * Sets minimum time milli-seconds between onCellInfoChanged
     */
     ...
     * @param p2 P2 parameter (described ISO 7816-4).
     ...
     *
     * @param subId The subscription to use.
     * @param contents  String containing SAT/USAT response hexadecimal
    ...
    /**
     * Read one of the NV items defined {@link RadioNVItems} / {@code ril_nv_items.h}.
    ...
    /**
     * Write one of the NV items defined {@link RadioNVItems} / {@code ril_nv_items.h}.
     * Used for device configuration by some CDMA operators.
     *
     * @param itemID the ID of the item to read.
    ...
    /**
     * Update the CDMA Preferred Roaming List (PRL) the radio NV storage.
     * Used for device configuration by some CDMA operators.
     *
     * @param preferredRoamingList byte array containing the new PRL.
     * @return true on success; false on any failure.
     */
    boolean nvWriteCdmaPrl(byte[] preferredRoamingList);
     ...
     * @param callingPackage The package making the call.
     *
     * @return the calculated preferred network type, defined RILConstants.java.
     */
     ...
     * @return the preferred network type, defined RILConstants.java.
     */
     ...
     */
    int requestNetworkScan(int subId, NetworkScanRequest request, Messenger messenger,
            IBinder binder);
     ...
     *        turned off? Will also result notification being not shown to
     ...
     *
     * @param subId the id of the subscription to update.
     * @param networkType the preferred network type, defined RILConstants.java.
     * @return true on success; false on any failure.
     */
     ...
     *
     * If any of the packages the calling UID has carrier privileges, the
     ...
     *
     * @param subId The subscription to use.
     * @return carrier privilege status defined TelephonyManager.
     */
     ...
     * for display purpose only, for example, displayed Phone Status. It won't
     * change the actual MSISDN/MDN. To unset alphatag or number, pass a null
     * value.
     *
     ...
     *
     * Once set, whenever the SIM is present the device, the service
    ...
    /**
     * Returns the URI for the per-account voicemail ringtone set Phone settings.
     *
    ...
    /**
     * Returns whether vibration is set for voicemail notification Phone settings.
     *
     * @param accountHandle The handle for the {@link PhoneAccount} for which to retrieve the
    ...
    */
     ...
     * carriers on success.
     */
    int setAllowedCarriers(int slotIndex, List<CarrierIdentifier> carriers);
    ...
    /**
     * Check if phone is emergency callback mode
     * @return true if phone is emergency callback mode
     * @param subId the subscription ID that this action applies to.
     * @hide
     */
