...
interface IBackupManager {
     ...
     */
    void initializeTransports(String[] transportNames, IBackupObserver observer);
     ...
     * backup dataset or from the restore set used the last wholesale restore operation,
     ...
     *     installed applications' data, not just those named the <code>packageNames</code>
     *     parameter.
     ...
     *     applications will be included the dataset.
    ...
    void adbBackup(ParcelFileDescriptor fd, boolean includeApks, boolean includeObbs,
            ...
            boolean doCompress, boolean doKeyValue, String[] packageNames);
     ...
     */
    void fullTransportBackup(String[] packageNames);

    /**
     * Restore device content from the data stream passed through the given socket.  The
     * data stream must be the format emitted by adbBackup().
     ...
     * full-device encryption password order to perform a backup.  If a password is
    ...
    void acknowledgeFullBackupOrRestore(int token, boolean allow,
            String curPassword, String encryptionPassword,
     ...
     * @hide
     */
    void selectBackupTransportAsync(ComponentName transport, ISelectBackupTransportCallback listener);

    /**
     * Get the configuration Intent, if any, from the given transport.  Callers must
     * hold the android.permission.BACKUP permission order to use this method.
     *
    ...
    /**
     * Bega restore session.  Either or both of packageName and transportID
     ...
     * withrequiring the app to hold any special permission.
     *
     * @param packageName The name of the single package for which a restore will
     *        be requested.  May be null, which case all packages the restore
     *        set can be restored.
     * @param transportID The name of the transport to use for the restore operation.
     *        May be null, which case the current active transport is used.
     ...
     *        total file size that would result from the operation. Unused all other
     *        cases.
     * {@hide}
     */
     ...
     * operation.
     *
     * @param monitor the {@link BackupManagerMonitor} to receive callbacks abimportant events
     ...
     */
    int requestBackup(String[] packages, IBackupObserver observer, IBackupManagerMonitor monitor,
        int flags);
